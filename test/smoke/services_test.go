	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			support.WithNamespace(t, cluster, func(env *support.Env) {
				...
				if tc.zitadel != nil {
					assert.AssertPartial(t, env.GetService(t, releaseName), *tc.zitadel, releaseName)
				}
				if tc.login != nil {
					assert.AssertPartial(t, env.GetService(t, releaseName+"-login"), *tc.login, releaseName+"-login")
				}
			})
		})
	}
