...
func TestDeploymentMatrix(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "login-externaldomain-auto-inject",
			setValues: map[string]string{
				"login.enabled":                          "true",
							...
							Containers: assert.Some([]assert.ContainerAssertion{
								{
									...
									Env: assert.Matching[[]assert.EnvVarAssertion](gomega.SatisfyAll(
										...
										gomega.WithTransform(func(envVars []corev1.EnvVar) int {
											count := 0
											for _, e := range envVars {
												if e.Name == "ZITADEL_EXTERNALDOMAIN" {
													count++
												}
											}
											return count
		...
		{
			name: "login-externaldomain-no-duplicate",
			setValues: map[string]string{
				"login.enabled":                          "true",
				"zitadel.configmapConfig.ExternalDomain": "auth.example.com",
				"login.env[0].name":                      "ZITADEL_EXTERNALDOMAIN",
				"login.env[0].value":                     "custom.auth.example.com",
			},
			login: &assert.DeploymentAssertion{
				Spec: assert.DeploymentSpecAssertion{
					Template: assert.PodTemplateSpecAssertion{
