...
| caBundleSecret | string | `""` | Name of a Kubernetes Secret containing a custom CA bundle for TLS verification. The secret must contain a key "ca.crt" with PEM-encoded CA certificates. When set, the bundle is mounted into both the ZITADEL and Login containers and SSL_CERT_FILE is configured to use it. This allows containers to trust custom CAs for internal communication (e.g., when using serverSslCrtSecret with certificates signed by a private CA). Note: SSL_CERT_FILE replaces the default system CA bundle for the process. If the containers also need to trust public CAs, "ca.crt" should contain a full CA bundle (system roots + your custom roots), not just the private CA. |
