...
| zitadel.adminServiceKeySecretName | string | `""` | Name of an existing Kubernetes Secret containing the admin service keypair. The secret must contain keys "tls.crt" (RSA public certificate) and "tls.key" (RSA private key). When set, the chart skips auto-generating a keypair and uses this secret instead. Useful for cert-manager or external secret operators. The public cert is mounted as ZITADEL_ADMINCLIENT_KEYFILE to register an admin-client system user with IAM_OWNER role. The private key can be extracted by operators to sign JWTs for admin API access. |
