...
| caBundleSecret | string | `""` | Name of a Kubernetes Secret containing a custom CA bundle for TLS verification. The secret must contain a key "ca.crt" with PEM-encoded CA certificates. When set, the bundle is mounted into /etc/ssl/certs/ in both the ZITADEL and Login containers via subPath, and SSL_CERT_DIR is set to /etc/ssl/certs. This extends the system trust store with custom CAs rather than replacing it, so public CA trust is preserved. Use this to trust custom CAs for internal communication (e.g., when using serverSslCrtSecret with certificates signed by a private CA). |
