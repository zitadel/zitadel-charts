...
func (s *ConfigurationTest) checkAccessibility(pods []corev1.Pod) {
	...
	checks := append(
		zitadelStatusChecks(8080),
		&httpCheckOptions{
			getUrl: Audience() + "/ui/console/assets/environment.json",
			test: func(resp *http.Response, body []byte) error {
				...
				for _, expect := range []string{
					fmt.Sprintf(`"api":"%s"`, aud),
					fmt.Sprintf(`"issuer":"%s"`, aud),
				} {
