...
import (
	...
	mgmt_client "github.com/zitadel/zitadel-go/v2/pkg/client/management"
...
func testAuthenticatedAPI(secretName, secretKey string) func(test *acceptance.ConfigurationTest) {
	return func(cfg *acceptance.ConfigurationTest) {
		...
		jwta, err := oidc.NewJWTProfileAssertionFromFileData(key, []string{acceptance.Audience()})
		if err != nil {
			t.Fatal(err)
		}
		...
		acceptance.Await(cfg.Ctx, t, nil, 60, func(ctx context.Context) error {
			var tokenErr error
			token, tokenErr = getToken(ctx, t, jwt)
			return tokenErr
		})
		acceptance.Await(cfg.Ctx, t, nil, 60, func(ctx context.Context) error {
			if httpErr := callAuthenticatedHTTPEndpoint(ctx, token); httpErr != nil {
				return httpErr
			}
			return callAuthenticatedGRPCEndpoint(ctx, key)
		})
	}
}

func getToken(ctx context.Context, t *testing.T, jwt string) (string, error) {
	form := url.Values{}
	...
	//nolint:bodyclose
	resp, tokenBody, err := acceptance.HttpPost(ctx, fmt.Sprintf("%s/oauth/v2/token", acceptance.Audience()), func(req *http.Request) {
...
func callAuthenticatedHTTPEndpoint(ctx context.Context, token string) error {
	checkCtx, checkCancel := context.WithTimeout(ctx, 5*time.Second)
	defer checkCancel()
	//nolint:bodyclose
	resp, _, err := acceptance.HttpGet(checkCtx, fmt.Sprintf("%s/management/v1/languages", acceptance.Audience()), func(req *http.Request) {
...
func callAuthenticatedGRPCEndpoint(ctx context.Context, key []byte) error {
	conn, err := mgmt_client.NewClient(
		acceptance.Audience(),
		fmt.Sprintf("%s:%d", acceptance.Domain, acceptance.Port),
